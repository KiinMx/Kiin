...
{versión\_base}\_{DD.MM.YYYY}          → sin sufijo de versión

{versión\_base}\_{DD.MM.YYYY}\_{N}      → con sufijo de versión (N ≥ 1\)
...
interface CourseCSV {
  ...
  Horas\_a\_la\_semana: string

  Modalidad: string      // "Regular" o "Acompañamiento"

  Hr\_Pres: string        // Horas presenciales

  Hr\_Pres2: string

  Hr\_N\_P: string         // Horas no presenciales
  ...
  LIC\_MEFI: string       // Columnas MEFI por carrera
...
│   Fuente: public/data/data\_{label}\_{DD}.{MM}.{YYYY}\[\_{v}\].xlsx     │
